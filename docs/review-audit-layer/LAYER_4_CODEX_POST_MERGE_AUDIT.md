...
| production_integration | Production database, config, environment changes |
...
Every post-merge audit SHALL confirm and record these 22 mandatory fields grouped into seven evidence categories:
...
| Runtime evidence | `runtime_status`, `runtime_evidence_reference`, `runtime_reason`, `runtime_surface`, `runtime_evidence_or_substitute_evidence`, `runtime_verified_by`, `runtime_verified_at` | Records full canonical runtime evidence or substitute evidence. |
...
A post-merge audit that omits any of the 22 fields is incomplete. Gate 11 must verify one valid cleanup outcome before setting `lane_closure_ready: true`; `already_absent` and `not_applicable_with_reason` still require Gate 11 verification. Only Gate 11 permits `BRANCH_CLEANUP` to transition to `CLOSED`.
...
- Record all 22 mandatory post-merge fields
...
- Skip any of the 22 mandatory fields
