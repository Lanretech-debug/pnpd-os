...
| Entry Criteria | Gate 10 completed; all 17 mandatory post-merge fields are recorded; `blocking_findings` is empty; `next_state` is `BRANCH_CLEANUP`; cleanup eligibility is established; and `lane_closure_ready` remains `false`. Already-absent and not-applicable outcomes still require Gate 11 verification. Any blocking finding, regardless of severity label, forbids Gate 11. |
